...
export async function POST(request: Request) {
  try {
    ...
    const checkGe = (v: number, norm: number, inv = false) => {
      if (inv) return v <= norm ? 'соответствует норме' : 'превышает ориентир'
      return v >= norm ? 'соответствует норме' : 'ниже ориентира'
    }
...
ФОРМАТ: чистый текст без markdown. НЕ используй **, *, #, -, •, ►, ▪ и прочие спецсимволы. Заголовки разделов пиши обычным текстом с нумерацией.
