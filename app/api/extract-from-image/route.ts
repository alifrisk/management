...
- ВАЖНО: Балансовое равенство: Активы = Обязательства + Капитал. Разбивай статьи правильно.
- Если компонент баланса явно не указан — помести остаток в p_other_assets или p_other_liab

Соответствие полей статьям баланса:
АКТИВ:
  p_cash         = Денежные средства, наличные, корсчет в ЦБ, касса
  p_receivables  = Средства в банках, МБК размещённые, ностро счета
  p_investments  = Ценные бумаги, инвестиционный портфель, ГЦБ
  p_loans_issued = Кредитный портфель (нетто), ссуды клиентам, займы выданные
  p_fixed_assets = Основные средства, нематериальные активы, недвижимость
  p_other_assets = ВСЕ ОСТАЛЬНЫЕ активы (налоговые, прочие). Если сумма компонентов ≠ итогу — разницу сюда.
ПАССИВ:
  p_deposits     = Депозиты клиентов, вклады физлиц и юрлиц, счета клиентов
  p_borrowings   = МБК привлечённые, займы от банков, выпущенные облигации, субординированный долг
  p_other_liab   = Прочие обязательства, кредиторская задолженность. Если сумма ≠ итогу — разницу сюда.
  p_equity       = Собственный капитал = уставный капитал + резервы + нераспределённая прибыль
ОПУ:
  p_interest_income   = Процентные доходы (до вычета расходов)
  p_interest_expense  = Процентные расходы
  p_fee_income        = Комиссионные доходы (нетто)
  p_fx_income         = Доходы от валютных операций, курсовые разницы
  p_other_income      = Прочие операционные доходы
  p_operating_expense = Операционные расходы, административные расходы, зарплаты
  p_provisions        = Резервы на потери по кредитам, отчисления в резервы
  p_net_profit        = Чистая прибыль / убыток (итоговая строка ОПУ)
