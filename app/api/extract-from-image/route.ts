...
const CREDIT_COEFF_PROMPT = `Ты финансовый аналитик. На скриншоте таблица финансовых коэффициентов предприятия МСБ.
Извлеки значения и верни ТОЛЬКО JSON без пояснений.

ПРАВИЛА:
- Коэффициенты (Ктл, Кбл, Кфин, DSCR): число как есть: 1.80, 0.35, 1.20
- Проценты (ROA, ROE, покрытие залогом): число БЕЗ знака %: 8.1 (не "8.1%")
- "—", "-", "н/д", пусто → null
- p1 = более ранний период (левый столбец), p2 = более поздний (правый)
- Если виден только один период — заполни p2_, оставь p1_ = null

МАППИНГ ПОКАЗАТЕЛЕЙ:
- Ктл / коэффициент текущей ликвидности / current ratio → ctl_p1, ctl_p2
- Кбл / коэффициент быстрой ликвидности / quick ratio → kbl_p1, kbl_p2
- ROA / рентабельность активов → roa_p1, roa_p2  (число без %, напр. 8.1)
- ROE / рентабельность собственных средств → roe_p1, roe_p2  (число без %)
- Кфин / коэффициент финансирования / леверидж → kfin_p1, kfin_p2
- DSCR / коэффициент покрытия долга / debt service coverage → dscr_p1, dscr_p2
- Покрытие залогом / залоговое покрытие / collateral coverage → coll_cov_p1, coll_cov_p2  (число без %, напр. 150.0)
...
export async function POST(request: Request) {
  try {
    ...
    const prompt = module === 'credit' ? CREDIT_PROMPT : module === 'credit_coeff' ? CREDIT_COEFF_PROMPT : FINANCIAL_PROMPT
    ...
    // coefficient mode — keep floats, allow null
    const isCoeff = module === 'credit_coeff'
    const extracted: Record<string, unknown> = {}
    for (const [k, v] of Object.entries(raw)) {
      if (STRING_KEYS.has(k)) {
        extracted[k] = typeof v === 'string' ? v.trim() : ''
      } else if (isCoeff) {
        if (v === null || v === undefined || v === '') { extracted[k] = null; continue }
        const n = typeof v === 'number' ? v : parseFloat(String(v).replace(/[^0-9.\-]/g, ''))
        extracted[k] = isFinite(n) ? Math.round(n * 100) / 100 : null
      } else {
