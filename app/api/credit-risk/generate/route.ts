...
3-4 предложения. Прямо укажи: PAR30 общего портфеля банка ${bankPar30Violated ? `нарушается (${par30AfterBank.toFixed(2)}% > лимит ${raBankLimit}%)` : 'не нарушается'}. Покрытие залогом ${collateralViolated ? `ниже нормы (${effectiveCovPct.toFixed(1)}% < 100%)` : `в норме (${effectiveCovPct.toFixed(1)}%)`}. Прочие отклонения перечисли как замечания — они не изменяют вердикт.
