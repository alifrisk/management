...
Покрытие: ${collateral_coverage_existing.toFixed(1)}% ${collateral_coverage_existing > 200 ? '✓ норма' : collateral_coverage_existing >= 150 ? '⚠ допустимо' : '✗ ниже нормы'}
Норматив банка: >200%
` : is_increase ? `
...
Залог / Новый лимит: ${pct(collateral_coverage)} ${collateral_coverage > 200 ? '✓ норма' : collateral_coverage >= 150 ? '⚠ допустимо' : '✗ ниже нормы'}
` : ''}
...
${is_collateral_change
  ? `Раскрой:
- Покрытие залогом: ${collateral_coverage_existing.toFixed(1)}% (норма >200%) — оценка достаточности
