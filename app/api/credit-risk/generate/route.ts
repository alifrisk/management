...
export async function POST(request: Request) {
  try {
    ...
    // ── Manual coefficients from Tab 2 (primary financial data source) ──
    const mc = (fd.manual_coeffs || {}) as Record<string, number | string | null | undefined>
    const mcv = (v: unknown) => (v != null && v !== '' && isFinite(Number(v))) ? Number(v).toFixed(2) : 'н/д'
    const mcp = (v: unknown) => (v != null && v !== '' && isFinite(Number(v))) ? Number(v).toFixed(1) + '%' : 'н/д'
    const mcOk = (v: unknown, norm: unknown, def: string): string => {
      if (v == null || v === '' || !isFinite(Number(v))) return ''
      const n = String(norm || def)
      const m = n.match(/([<>])\s*([\d.]+)/)
      if (!m) return ''
      return (m[1] === '>' ? Number(v) > parseFloat(m[2]) : Number(v) < parseFloat(m[2])) ? ' ✓ норма' : ' ✗ ниже нормы'
    }
    const hasMC = ['ctl_p2','kbl_p2','roa_p2','roe_p2','kfin_p2','dscr_p2','coll_cov_p2']
      .some(k => mc[k] != null && mc[k] !== '')
...
СИСТЕМА РЕШЕНИЯ (строго соблюдать — отклонение недопустимо):
...
Все прочие нарушения (концентрация МСБ, PAR30 МСБ, леверидж, ликвидность, ROA, ROE) → решение НЕ МЕНЯЮТ, но ОБЯЗАТЕЛЬНО упоминаются в тексте как риски и замечания.
РЕШЕНИЕ СИСТЕМЫ: ${finalVerdict} — ты ОБЯЗАН указать именно это значение без изменений.

ВАЖНО: все финансовые коэффициенты введены аналитиком вручную — используй ТОЛЬКО эти значения, не пересчитывай и не оценивай их достоверность.
СТРОГО ЗАПРЕЩЕНО писать или перефразировать любые из следующих фраз:
  • «баланс не представлен» / «баланс/ОПУ/ОДДС не представлены»
  • «финансовая отчётность не предоставлена» / «отчётность отсутствует»
  • «финансовые данные отсутствуют» / «нет финансовых данных»
  • «не поддаётся расчёту» / «не поддаются расчёту» / «расчёт невозможен»
  • «данные недоступны» / «недостаточно данных для расчёта»
  • любые упоминания баланса, ОПУ, ОДДС, финансовой отчётности как источников данных
Если показатель равен «н/д» — просто пропусти его, не упоминай и не объясняй отсутствие.
...
═══ ФИНАНСОВЫЕ КОЭФФИЦИЕНТЫ (введены аналитиком, использовать точно) ═══
${hasMC ? `Периоды: ${mc.p1_label || 'П1'} → ${mc.p2_label || 'П2'}
Примечание: для DSCR, Кфин и Покрытия залогом доступно только значение на ${mc.p2_label || 'П2'} — сравнение «было → стало» по этим трём показателям НЕ СТРОИТЬ, значение П1 не придумывать.

ПОКАЗАТЕЛИ ЛИКВИДНОСТИ:
1. Коэффициент текущей ликвидности (КТЛ) [норма ${mc.ctl_norm || '>2.0 (>200%)'}]
   ${mc.p1_label || 'П1'}: ${mcv(mc.ctl_p1)}${mcOk(mc.ctl_p1, mc.ctl_norm, '>2.0')}
   ${mc.p2_label || 'П2'}: ${mcv(mc.ctl_p2)}${mcOk(mc.ctl_p2, mc.ctl_norm, '>2.0')}

2. Коэффициент быстрой ликвидности (КБЛ) [норма ${mc.kbl_norm || '>1.0 (>100%)'}]
   ${mc.p1_label || 'П1'}: ${mcv(mc.kbl_p1)}${mcOk(mc.kbl_p1, mc.kbl_norm, '>1.0')}
   ${mc.p2_label || 'П2'}: ${mcv(mc.kbl_p2)}${mcOk(mc.kbl_p2, mc.kbl_norm, '>1.0')}

ПОКАЗАТЕЛИ РЕНТАБЕЛЬНОСТИ:
3. Рентабельность активов (ROA) [норма ${mc.roa_norm || '>6%'}]
   ${mc.p1_label || 'П1'}: ${mcp(mc.roa_p1)}${mcOk(mc.roa_p1, mc.roa_norm, '>6')}
   ${mc.p2_label || 'П2'}: ${mcp(mc.roa_p2)}${mcOk(mc.roa_p2, mc.roa_norm, '>6')}

4. Рентабельность собственных средств (ROE) [норма ${mc.roe_norm || '>20%'}]
   ${mc.p1_label || 'П1'}: ${mcp(mc.roe_p1)}${mcOk(mc.roe_p1, mc.roe_norm, '>20')}
   ${mc.p2_label || 'П2'}: ${mcp(mc.roe_p2)}${mcOk(mc.roe_p2, mc.roe_norm, '>20')}

ПОКАЗАТЕЛИ ФИНАНСОВОЙ УСТОЙЧИВОСТИ:
5. Коэффициент финансирования (Кфин) [норма ${mc.kfin_norm || '>0.5'}] — только ${mc.p2_label || 'П2'}
   ${mc.p2_label || 'П2'}: ${mcv(mc.kfin_p2)}${mcOk(mc.kfin_p2, mc.kfin_norm, '>0.5')}

ПОКАЗАТЕЛИ КРЕДИТОСПОСОБНОСТИ:
6. Коэффициент покрытия долга (DSCR) [норма ${mc.dscr_norm || '>1.0'}] — только ${mc.p2_label || 'П2'}
   ${mc.p2_label || 'П2'}: ${mcv(mc.dscr_p2)}${mcOk(mc.dscr_p2, mc.dscr_norm, '>1.0')}

7. Покрытие залогом [норма ${mc.coll_cov_norm || '>120%'}] — только ${mc.p2_label || 'П2'}
   ${mc.p2_label || 'П2'}: ${mcp(mc.coll_cov_p2)}${mcOk(mc.coll_cov_p2, mc.coll_cov_norm, '>120')}
` : `Финансовые коэффициенты не введены аналитиком. Анализ финансовых показателей не проводить.`}
7-авто. Покрытие залогом (авто) = Залог / ${is_collateral_change ? 'Остаток кредита' : 'Кредит'} × 100% [норма >120%]
...
${cVio ? '⚠️ Нарушение лимита концентрации МСБ — отрази в тексте как замечание, решение НЕ МЕНЯЕТ.' : ''}${msbVio ? '⚠️ Нарушение PAR30 МСБ-портфеля — отрази в тексте как риск, решение НЕ МЕНЯЕТ.' : ''}${bankVio ? '❌ Нарушение PAR30 ОБЩЕГО портфеля банка — это условие для «Не рекомендуется», решение уже определено системой.' : ''}
`
...
` : ''}Напиши профессиональное заключение строго по следующей структуре (не более 550 слов). Используй разделители [РАЗДЕЛ] ТОЧНО как указано ниже.

[ХАРАКТЕРИСТИКА ЗАЁМЩИКА]
2-3 предложения: сектор бизнеса, опыт (${fd.years_in_business || '?'} лет), кредитная история, общая оценка надёжности клиента.

[ОБОСНОВАНИЕ ОПЕРАЦИИ]
${is_collateral_change
  ? `Объясни, почему смена залога по действующему кредиту (остаток ${f(existing_balance)} ${fd.loan_currency}) целесообразна. Оцени качество нового залога (${f(collateral_total)} TJS, покрытие ${collateral_coverage_existing.toFixed(1)}%), его ликвидность и риски обесценения. 2-3 предложения.`
  : is_increase
  ? `Объясни, почему увеличение лимита с ${f(existing_balance)} до ${f(loan_amount)} ${fd.loan_currency} (+${existing_balance > 0 ? (((loan_amount - existing_balance) / existing_balance) * 100).toFixed(1) : '—'}%) обосновано. Опирайся на динамику выручки/прибыли, цель использования, достаточность залога (${pct(collateral_coverage)}). 2-3 предложения.`
  : `Объясни, почему открытие кредитной линии на ${f(loan_amount)} ${fd.loan_currency} обосновано. Опирайся на финансовое состояние, специфику бизнеса, цель использования. 2-3 предложения.`}

[ОЦЕНКА РИСКОВ]
Ровно 3 риска. Каждый — строго по формату: «Название риска: обоснование (1 предложение). Уровень: высокий / средний / низкий.»${fd.concentration_violates || fd.par30_msb_violates || bankPar30Violated ? '\nОбязательно включи нарушения концентрации/PAR30 как один из рисков.' : ''}

[РЕШЕНИЕ И ОБОСНОВАНИЕ]
Решение системы: ${finalVerdict}.
3-4 предложения. Прямо укажи: PAR30 общего портфеля банка ${bankPar30Violated ? `нарушается (${par30AfterBank.toFixed(2)}% > лимит ${raBankLimit}%)` : 'не нарушается'}. Покрытие залогом ${collateralViolated ? `ниже нормы (${effectiveCovPct.toFixed(1)}% < 100%)` : `в норме (${effectiveCovPct.toFixed(1)}%)`}. Прочие отклонения перечисли как замечания — они не изменяют решение.

ПРАВИЛА ОФОРМЛЕНИЯ:
- НЕ указывай конкретные даты (числа, месяцы, годы) в тексте — дата проставляется системой
- НЕ используй markdown: запрещены #, *, **, _, тире в начале строк как маркеры
- Разделители [ХАРАКТЕРИСТИКА ЗАЁМЩИКА], [ОБОСНОВАНИЕ ОПЕРАЦИИ], [ОЦЕНКА РИСКОВ], [РЕШЕНИЕ И ОБОСНОВАНИЕ] — писать ТОЧНО без изменений

Завершить ТОЧНО так:
Руководитель СУР — Сангинова Ф.
РЕКОМЕНДАЦИЯ: ${finalVerdict}
УРОВЕНЬ РИСКА: напиши ОДНО слово — Низкий, Средний или Высокий`
    ...
    // Решение всегда определяется системой; парсим AI-вывод только как проверку
    ...
    const recommendation = rawRec.includes('Не рекомендуется') ? 'Не рекомендуется'
      ...
      : finalVerdict  // если AI вдруг использовал другую формулировку — берём системное решение
    const rawRisk = riskMatch ? riskMatch[1].trim() : ''
    const risk_level = rawRisk.includes('Высокий') ? 'Высокий'
      : rawRisk.includes('Низкий') ? 'Низкий'
      : 'Средний'
