...
export async function POST(request: Request) {
  try {
    ...
    const conclusionItems: (Paragraph | Table)[] = [
      ...
      addPara(
        `T+1 (1 день): покрытие ${(t.coverage_cash_t1 * 100).toFixed(1)}% — ${t1ok ? 'ПРОФИЦИТ' : 'ДЕФИЦИТ'}. ` +
        (t1ok ? 'Буфер ликвидности достаточен для покрытия однодневных обязательств.'
               : `Дефицит ${new Intl.NumberFormat('ru-RU').format(Math.round(t.need_t1 - t.cash_equivalents))} TJS.`),
        false, t1ok ? '1B8A4C' : 'C00000'
      ),
      addPara(
        `T+7 (7 дней): покрытие ${(t.coverage_cash_t7 * 100).toFixed(1)}% — ${t7ok ? 'ПРОФИЦИТ' : 'ДЕФИЦИТ'}. ` +
        (t7ok ? 'Семидневный горизонт покрывается имеющимися ресурсами.'
               : `Необходимо дополнительное фондирование ${new Intl.NumberFormat('ru-RU').format(Math.round(t.need_t7 - t.cash_equivalents))} TJS.`),
        false, t7ok ? '1B8A4C' : 'C00000'
      ),
      addPara(
        `T+30 (30 дней): покрытие ${(t.coverage_cash_t30 * 100).toFixed(1)}% — ${t30ok ? 'ПРОФИЦИТ' : 'ДЕФИЦИТ'}. ` +
        (t30ok ? 'Банк устойчив в 30-дневном горизонте при стрессовом сценарии.'
               : `Существенный дефицит ${new Intl.NumberFormat('ru-RU').format(Math.round(t.need_t30 - t.cash_equivalents))} TJS требует мер.`),
      ...
      sections: [{
        ...
        children: [
          ...
          ...conclusionItems,om 'next/server'
import {
  Document, Packer, Paragraph, TextRun, Table, TableRow, TableCell,
  AlignmentType, BorderStyle, WidthType, VerticalAlign, ShadingType, LevelFormat
} from 'docx'
