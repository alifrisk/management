export default function Page() {
  return (
    <div className="max-w-6xl mx-auto">
      <div className="bg-white rounded-xl border border-gray-100 p-8 shadow-sm text-center">
        <div className="w-14 h-14 bg-green-50 rounded-xl flex items-center justify-center mx-auto mb-4">
          <span className="text-2xl">🔧</span>
        </div>
        <h1 className="text-xl font-semibold text-gray-900 mb-2">Рыночный риск — Оценка контрагентов</h1>
        <p className="text-gray-500 text-sm">Модуль в разработке — следующие спринты ✅</p>
      </div>
    </div>
  )
}
