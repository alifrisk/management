...
function FinRow({ label, f1, f2, bold, auto, v1, v2, formData, setF }: FinRowProps) {
...
function FinTable({ title, p1label, p2label, children }: FinTableProps) {
  return (
    <div className="mb-4">
      ...
      <table className="w-full border border-gray-200 rounded-lg overflow-hidden text-sm">
        <thead>
          <tr className="bg-[#1B8A4C] text-white">
            ...
            <th className="text-left px-3 py-2 text-xs font-medium w-1/4">{p1label || 'Период 1'}</th>
            <th className="text-left px-3 py-2 text-xs font-medium w-1/4">{p2label || 'Период 2'}</th>
          </tr>
        </thead>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[95vh] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-6">
              ...
              p1label={formData.p1_label || 'Период 1'}
              p2label={formData.p2_label || 'Период 2'}>
                    <FinRow label="Денежные средства" f1="p1_cash" f2="p2_cash"
                    formData={formData} setF={setF} />
